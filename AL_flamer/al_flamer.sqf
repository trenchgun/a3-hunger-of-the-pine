...
while {alive _flamer} do 
{
	...
	{
		// ACE setDamage alternatives:
		// ----------------------------------------------
		// [player, ((getdammage player) + _damage_strig), "body", "unknown"] call ace_medical_fnc_addDamageToUnit;
		// [_x, (getdammage _x + 0.05), "body", "unknown"] call ace_medical_fnc_addDamageToUnit;
		// ----------------------------------------------
		...
		_nearflamer = (ASLToAGL getPosASL _flamer) nearEntities ["CAManBase",5]; {[_x, (getdammage _x + 1), "body", "unknown"] call ace_medical_fnc_addDamageToUnit; _tip = selectrandom ["04","burned","02","03"]; [_x,[_tip,200]] remoteExec ["say3d"]} forEach (_nearflamer-[_flamer]);
		...
		else {_nearflamer = (ASLToAGL getPosASL _flamer) nearEntities ["CAManBase",5]; {[_x, (getdammage _x + 1), "body", "unknown"] call ace_medical_fnc_addDamageToUnit} forEach (_nearflamer-[_flamer]); [[_flamer],"AL_flamer\flamer_jump_SFX.sqf"] remoteExec ["execvm"]; [_flamer,_tgt_flamer,_cap_flamer] spawn fnc_jump_flamer};
